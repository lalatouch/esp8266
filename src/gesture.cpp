...
namespace gestureNS {
    ...
    void addNewDataPoint(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz){
        ...
        // Store the point only if we are already listening to a gesture or it might be the beginning of a gesture
    ...
    const bool isRotationGesture(){
        ...
        if (rotationNormSum > rotationNormThresholdPercent * currentGesture.size()/4 * rotationNormThreshold) {
            return true;
        }
        return false;
    }
    ...
    vector<vector<float> > computeAccelerationDifferences(vector<vector<float> > accelerationPoints){
        ...
        for (int i=0; i<accelerationPoints.size()-1; i++){
            newPoint = { accelerationPoints[i+1][0]-accelerationPoints[i][0], accelerationPoints[i+1][1]-accelerationPoints[i][1] };
    ...
    vector<vector<float> > normalize2DVector(vector<vector<float> > points){
        ...
        for (int i=0; i<points.size(); i++){
            float squareNorm = (points[i][0]*points[i][0])+(points[i][1]*points[i][1]);
            newPoint = {points[i][0]/sqrt(squareNorm), points[i][1]/sqrt(squareNorm)};
            result.push_back(newPoint);
        }
        return result;
    }
    ...
    const bool isLineGesture(vector<vector<float> > normalizedAccelerationPoints){
        ...
        for (int i=0; i<pointsSize-1; i++){
            sumScalarProducts += abs(normalizedAccelerationPoints[i][0]*normalizedAccelerationPoints[i+1][0] + normalizedAccelerationPoints[i][1]*normalizedAccelerationPoints[i+1][1]);
        }
        if (sumScalarProducts > pointsSize - sumScalarProductsThreshold){
            return true;
        }
        return false;
    }
    ...
    bool isUnidirectionalGesture(vector<vector<float> > accelerationPoints){
        ...
        float scalarProduct = sumFirstPoints[0]*sumLastPoints[0] + sumFirstPoints[1]*sumLastPoints[1];
